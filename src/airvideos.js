...
//------------------------------------------
function AirVideos_commandFromServer(command, p1, p2) {
    switch (command) {
        case "UPLOADED":
        ...
        case "CREATED_NEW_CHANNEL":
            const ba = mapBaseAppID_to_AirVideo.get(p1)
...
function shareLinkToObject(baseAppID = "", url, doConfirm) {
    ...
    if (ba.appInfo.gamingPath)
        s += " &nbsp; <label> <input onClick='gamingMode=!gamingMode;shareLinkToObject(\"" + baseAppID + "\")' type='checkbox' " + (gamingMode ? "checked" : "") + ">gaming </label>"
...
function fromYTcodesToSet(YTcodes)
{
    ...
    {
        const pos = YTcodes.indexOf(' ', lastPos)
        if (pos === -1)
            pos = YTcodes.length
